...
if ((isset($path[3]) ? ($path[3]=="crud" ? True : False) : False)) {	# Nivel 2
	$p1->assign("crud", $path[4]);
	switch ($path[4]) {
		case 'u':	// Editar
			$sql="SELECT id,nombre,cuenta,foto,estado,perfil_id FROM usuarios WHERE id=".$path[5];
			$boton="Actualizar";
